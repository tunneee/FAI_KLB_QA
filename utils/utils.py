...
try:
    firebase_admin.initialize_app(cred, {'storageBucket': 'trifecta-8f603.appspot.com'})
except:
...
def generate_keyframes_and_upload_to_db(path, filename, vid_id, vid_url, framerate, ):
...
def upload_new_videos_to_db(path, file):
    ...
    keyframes_url, keyframes_uuid = generate_keyframes_and_upload_to_db(path, file, vid_id, url, fps)
    client.upsert(
        collection_name=collection_name,
        points=[
            models.PointStruct(
